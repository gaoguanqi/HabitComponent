package com.maple.reslibrary.widget.indicator;
...
import com.maple.reslibrary.R;
import com.maple.reslibrary.utils.LogUtils;
...
public class MapleIndicator extends View {
    ...
    private static final int SPLIT_OFFSET = SizeUtils.dp2px(10);
    ...
    public MapleIndicator(Context context) {
        this(context, null);
    }

    public MapleIndicator(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MapleIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.MapleIndicator);
        indicatorType = typedArray.getInt(R.styleable.MapleIndicator_indicatorType, 0);
        normalPointRadius = typedArray.getDimensionPixelSize(R.styleable.MapleIndicator_normalPointRadius, DEFAULT_NORMAL_POINT_RADIUS);
        selectedPointRadius = typedArray.getDimensionPixelSize(R.styleable.MapleIndicator_selectedPointRadius, indicatorType == INDICATOR_TYPE_GRADUAL ? DEFAULT_NORMAL_POINT_RADIUS : DEFAULT_SELECTED_POINT_RADIUS);
        pointInterval = typedArray.getDimensionPixelSize(R.styleable.MapleIndicator_pointInterval, 20);
        normalPointColor = typedArray.getColor(R.styleable.MapleIndicator_normalPointColor, Color.parseColor("#FFFFFF"));
        selectedPointColor = typedArray.getColor(R.styleable.MapleIndicator_selectedPointColor, Color.parseColor("#11EEEE"));
        typedArray.recycle();
        ...
        if (viewPager != null) {
            viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
                ...
                public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                    LogUtils.logGGQ("posInfo: " + currentPagePosition + " " + position + " " + positionOffset + " " + positionOffsetPixels);
                    // 动态计算当前页与目标页位置
...
//                    selectedPagePosition = position;
    ...
    protected void onDraw(Canvas canvas) {
        if (pointCount <= 0) {
            LogUtils.logGGQ("adapter size is 0");
            return;
        }
